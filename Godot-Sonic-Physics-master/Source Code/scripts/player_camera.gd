...
onready var camera_scroll: Node2D = $CameraScroll
var scroll_timer: float

func can_scroll(delta: float) -> bool:
	if scroll_timer > 0:
		scroll_timer -= delta
		return false
	return true

func camera_step(player: PlayerPhysics, delta: float) -> void:
	horizontal_border(player)
	vertical_border(player)
	vertical_scrolling(player, delta)

func horizontal_border(player: PlayerPhysics) -> void:
...
func vertical_border(player: PlayerPhysics) -> void:
...
func vertical_scrolling(player: PlayerPhysics, delta: float) -> void:
